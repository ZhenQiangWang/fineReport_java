 ...
 * @author fanruan
 */
public class TestParamTableDataDemo extends AbstractTableData {
    /**
     * 列名数组，保存程序数据集所有列名
     */
    ...
    public TestParamTableDataDemo() {
        ...
        parameterProviders[0] = new Parameter("columnNames");
        setDefaultParameters(parameterProviders);

        String property = this.parameters.getProperty();
        FineLoggerFactory.getLogger().error("consu1。。。。。。。。。。。。。。。。。。"+property);

        Object[] objects = this.parameters.get().toArray();
        FineLoggerFactory.getLogger().error("consu2。。。。。。。。。。。。。。。。。。"+ Arrays.toString(objects));
    }
